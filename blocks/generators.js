...
Blockly.Python['delay_pin_list'] = function (block) {
  ...
  while (block.getInput('ADD' + i)) {
    const v = Blockly.Python.valueToCode(
      block, 'ADD' + i, Blockly.Python.ORDER_ATOMIC
    ) || '0';
    items.push(v);
    i++;
  }
...
Blockly.Python['delay_turn_on'] = function (block) {
  const ch = Blockly.Python.valueToCode(
    block, 'channel', Blockly.Python.ORDER_ATOMIC
  ) || '1';
...
Blockly.Python['delay_turn_on_multiple'] = function (block) {
  const chs = Blockly.Python.valueToCode(
    block, 'channels', Blockly.Python.ORDER_ATOMIC
  ) || '[]';
